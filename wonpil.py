import streamlit as st
import pandas as pd
import plotly.express as px
import urllib.parse

# --- 配置區 ---
SHEET_ID = "11UXviXGiGJ33aRss2TdIL5b57vp8jrvqvjclK-TPkY8"
SHEET_NAME = "工作表1"

def get_data():
    encoded_sheet_name = urllib.parse.quote(SHEET_NAME)
    url = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&sheet={encoded_sheet_name}"
    
    df = pd.read_csv(url)
    
    # 強制修正欄位名稱，移除所有空格，避免找不到 '變化量'
    df.columns = [c.strip() for c in df.columns]
    
    # 處理時間格式 (處理上午/下午問題)
    df['時刻'] = df['時刻'].astype(str).str.replace('下午', ' PM').str.replace('上午', ' AM')
    df['時刻'] = pd.to_datetime(df['時刻'], errors='coerce')
    
    # 自動尋找「變化量」欄位 (增加容錯率)
    diff_col = next((c for c in df.columns if '變' in c), '變化量')
    total_col = next((c for c in df.columns if '總' in c), '總銷量')

    # 清洗數據：轉為數字
    df[diff_col] = df[diff_col].astype(str).str.replace('+', '', regex=False).replace('紀錄更新', '0')
    df[diff_col] = pd.to_numeric(df[diff_col], errors='coerce').fillna(0)
    df[total_col] = pd.to_numeric(df[total_col], errors='coerce').fillna(0)
    
    # 為了程式後續統一，重新命名
    df = df.rename(columns={diff_col: '變化量', total_col: '總銷量'})
    
    return df.dropna(subset=['時刻']).sort_values('時刻')

st.set_page_config(page_title="WONPIL 銷量實時監控", page_icon="📈", layout="wide")
st.title("📊 WONPIL 簽售銷量實時監控站")

try:
    df = get_data()
    
    if not df.empty:
        latest = df.iloc[-1]
        c1, c2, c3 = st.columns(3)
        c1.metric("目前總銷量", f"{int(latest['總銷量'])} 張")
        c2.metric("最後更新時間", latest['時刻'].strftime('%m/%d %H:%M:%S'))
        c3.metric("上次增幅", f"+{int(latest['變化量'])} 張")

        # --- 購買力單次排名 (Top 10) ---
        st.subheader("🔥 購買力單次排名 (Top 10)")
        # 排除 0 的紀錄，按變化量降序排
        rank = df[df['變化量'] > 0].sort_values('變化量', ascending=False).head(10).copy()
        if not rank.empty:
            rank['時刻'] = rank['時刻'].dt.strftime('%m/%d %H:%M')
            display_df = rank[['時刻', '總銷量', '變化量']].reset_index(drop=True)
            display_df.index += 1
            st.table(display_df)
        else:
            st.write("目前尚未偵測到銷量變動。")

        # --- 趨勢圖 ---
        st.subheader("📈 銷量增長趨勢")
        fig = px.line(df, x='時刻', y='總銷量', markers=True)
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("資料讀取中，請確保試算表已有內容...")

except Exception as e:
    st.error(f"解析失敗：可能是試算表欄位名稱不符")
    st.write(f"系統訊息：{e}")
    # 輔助偵測：列出目前讀到的欄位名
    if 'df' in locals():
        st.write(f"目前讀到的欄位有：{list(df.columns)}")

if st.button("手動重新整理"):
    st.rerun()
